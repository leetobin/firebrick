...
# check if array is already assembled and stop it
if test -d "/sys/block/md" ; then
	umount /md0p1
	mdadm --stop /dev/md0
fi  

# check if /dev/sda and /dev/sdb are present and the same size
if ( test -d "/sys/block/sda" ) && ( test -d "/sys/block/sdb" ) ; then
	read sdaSize < "/sys/block/sda/size"
	read sdbSize < "/sys/block/sdb/size"
	if [ "$sdaSize" == "$sdbSize" ] ; then
		...
		if [ "$ans" == "YES" ] ; then 
			...
			yes | mdadm --create /dev/md0 --level=0 --raid-devices=2 /dev/sda /dev/sdb
